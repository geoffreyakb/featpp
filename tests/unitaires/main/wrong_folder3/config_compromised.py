...
def scenario_cinq_tentatives(project_env):
    ...
    results["scenario_cinq_tentatives"] = [result, Penalty("Pénalité de 3 points", 3)]
...
def scenario_zero_tentative(project_env):
    ...
    results["scenario_zero_tentative"] = [result, Penalty("Pénalité de 1 point", 1)]
...
scenario = Scenario(scenario_test, _delay=0, _mark = 1)
scenario2 = Scenario(scenario_test2, _delay=0, _mark = 5)
scenario_big_delay = Scenario(scenario_delai_enorme, _delay = 24*3600*100, _mark = 3)
scenario_five_attempts = Scenario(scenario_cinq_tentatives, _delay=0, _nb_attempts = 5, _mark = 8)
scenario_zero_attempt = Scenario(scenario_zero_tentative, _delay=0, _nb_attempts = 0, _mark=3)
